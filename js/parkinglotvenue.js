$(document).ready(function() {
  ...
  var slotownersRef = firebase.firestore().collection("slotowners");
...
function loadData() {
  ...
  z = 1;
  slotownersRef.orderBy('name', "asc").get().then(function(querySnapshot) {
    ...
    $('#parkingvenueTable').on('click', '.btnDelete', function(){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          slotownersRef.doc(id).delete().then(function() {
    ...
    $('#changeData').on('submit', function(e) {
      ...
      slotownersRef.where("name", "==", name).get().then(function(querySnapshot) {
        if (querySnapshot.empty || (querySnapshot.docs.length === 1 && querySnapshot.docs[0].id === currentid)) {
          slotownersRef.doc(currentid).update({
            name: name,
            address: address
          }).then(function() {
            Swal.fire({
...
function checkSlotAvailability(name) {
  ...
  var query = slotownersRef.where("name", "==", name);
